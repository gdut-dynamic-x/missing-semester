...
## 自述文件
1、粗略地学习了Linux操作系统，大致了解了Linux系统的文件系统和权限机制及一些常用命令。
2、学习了shell脚本。
3、能够使用ls、cd、rm、mkdir、rmdir、pwd、touch等常用命令对Linux的文件或目录进行系列操作。
4、学会使用sudo以管理员身份进行操作，学会登陆root用户。
5、学会用grep、find等命令查找文件，用chmod命令修改文件rwx权限。
6、学会执行脚本文件。
7、学会用vim编辑文件，能够使用vim编辑器的一些基本操作。
8、学习了shell脚本中的一些工具，像管道、通配、重定向输出等。
9、学习了给文件定别名，学习了shell的命令行环境。
10、学习了简单的数据处理和git版本控制。
